...
echo $tis_i
echo $tissue
mkdir ./output_SMR_pcg_PGRP/${tissue}
...
{                     # <<<--- 添加该行
    ...
    smr --bfile ${bfile} --gwas-summary ${gf} --beqtl-summary ${besdprefix} --diff-freq 0.5 --diff-freq-prop 0.05 --out ./output_SMR_pcg_PGRP/${tissue}/${tissue}.pcg.${trait} --thread-num 23
    echo "" >&9      # <<<--- 添加该行
} &                  # <<<--- 添加该行
done
wait                 # <<<--- 添加该行
